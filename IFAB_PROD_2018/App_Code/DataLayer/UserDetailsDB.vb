...
Public Class UserDetailsDB
    ...
    Shared Function GetLinkName() As System.Data.DataTable
        ...
        Try
            ds = SqlHelper.ExecuteDataset(connectionString, CommandType.StoredProcedure, "Proc_GetCutomizeLinkName", arParms)
        Catch ex As Exception
            MsgBox(ex.ToString)
        End Try
        Return ds.Tables(0)
    ...
    Shared Function GetParentsName() As System.Data.DataTable
        ...
        Try
            ds = SqlHelper.ExecuteDataset(connectionString, CommandType.StoredProcedure, "Proc_GetParentsName")
        Catch ex As Exception
            MsgBox(ex.ToString)
        End Try
        Return ds.Tables(0)
    ...
    Shared Function CheckSecurityCheck() As String
        ...
        Return ds
    End Function
    'Shared Function GetHelpLink(ByVal Code As Integer) As String
    '    Dim connectionString As String = ConfigurationManager.ConnectionStrings("Advant").ConnectionString
    '    Dim ds As String
    '    Dim arParms() As SqlParameter = New SqlParameter(0) {}
    '    arParms(0) = New SqlParameter("@Code", SqlDbType.VarChar, 50)
    ...
    Shared Function GetBuyerOnlineInfo(ByVal UserID As String, ByVal PassWord As String) As System.Data.DataTable
        Dim connectionString As String = ConfigurationManager.ConnectionStrings("Advant").ConnectionString
        Dim ds As New DataSet
        Dim arParms() As SqlParameter = New SqlParameter(1) {}
        ...
        Try
            ds = SqlHelper.ExecuteDataset(connectionString, CommandType.StoredProcedure, "Proc_GetBuyerOnlineInfo", arParms)
        Catch ex As Exception
            MsgBox(ex.ToString)
        End Try
        If ds.Tables.Count = 0 Then
    ...
    Public Shared Function buyerinsert(ByVal usrid As Integer, ByVal imei As String) As System.Data.DataTable
        Dim connectionString As String = ConfigurationManager.ConnectionStrings("Advant").ConnectionString
        Dim ds As New DataSet
        Dim arParms() As SqlParameter = New SqlParameter(1) {}
        ...
        arParms(0) = New SqlParameter("@UserID", SqlDbType.Int)
        arParms(0).Value = usrid
        arParms(1) = New SqlParameter("@imei", SqlDbType.Text)
        arParms(1).Value = imei

        Try
            ds = SqlHelper.ExecuteDataset(connectionString, CommandType.StoredProcedure, "Proc_SaveBUYERLOG", arParms)
        Catch ex As Exception
            MsgBox(ex.ToString)
        End Try
    ...
    Public Shared Function DuplicateUser(ByVal usrid As String) As System.Data.DataTable
        Dim connectionString As String = ConfigurationManager.ConnectionStrings("Advant").ConnectionString
        Dim ds As New DataSet
        Dim arParms() As SqlParameter = New SqlParameter(0) {}

        arParms(0) = New SqlParameter("@UserID", SqlDbType.VarChar, 50)
        arParms(0).Value = usrid

        Try
            ds = SqlHelper.ExecuteDataset(connectionString, CommandType.StoredProcedure, "Proc_DuplicateBuyer", arParms)
        Catch ex As Exception
            MsgBox(ex.ToString)
        End Try
    ...
    Public Shared Function UpdateBuyerlog(ByVal Data As String) As Int16
        Try
            Dim connectionString As String = ConfigurationManager.ConnectionStrings("Advant").ConnectionString
            Dim rowsAffected As Integer = 0
            Dim arParms() As SqlParameter = New SqlParameter(0) {}
            ...
            'Begin Edit by Kusum
            arParms(0) = New SqlParameter("@LogID", SqlDbType.BigInt)
            arParms(0).Value = CInt(Data)
            'End Edit by Kusum

            Try
                rowsAffected = SqlHelper.ExecuteNonQuery(connectionString, CommandType.StoredProcedure, "Proc_UpdateBuyerUsers", arParms)
            Catch ex As Exception
                MsgBox(ex.ToString)
            End Try
            Return rowsAffected
    ...
    Public Shared Function Checkimei(ByVal imei As String) As System.Data.DataTable
        Dim connectionString As String = ConfigurationManager.ConnectionStrings("Advant").ConnectionString
        Dim ds As New DataSet
        Dim arParms() As SqlParameter = New SqlParameter(0) {}
        arParms(0) = New SqlParameter("@imei", SqlDbType.VarChar, 100)
        arParms(0).Value = imei
        Try
            ds = SqlHelper.ExecuteDataset(connectionString, CommandType.StoredProcedure, "Proc_Checkimei", arParms)
        Catch ex As Exception
            MsgBox(ex.ToString)
        End Try
    ...
    Public Shared Function Status() As System.Data.DataTable
        Dim connectionString As String = ConfigurationManager.ConnectionStrings("Advant").ConnectionString
        Dim ds As New DataSet
        Try
            ds = SqlHelper.ExecuteDataset(connectionString, CommandType.StoredProcedure, "Proc_Status")
        Catch ex As Exception
            MsgBox(ex.ToString)
        End Try
        ...
        Return ds.Tables(0)
    End Function
    Shared Function GetBuyerStatus(ByVal BuyerID As String, ByVal Branchcode As String) As System.Data.DataTable
        Dim connectionString As String = ConfigurationManager.ConnectionStrings("Advant").ConnectionString
        Dim ds As New DataSet
        Dim arParms() As SqlParameter = New SqlParameter(1) {}
        arParms(0) = New SqlParameter("@BuyerID", SqlDbType.Int)
        arParms(0).Value = BuyerID
        arParms(1) = New SqlParameter("@BranchCode", SqlDbType.VarChar, 100)
        arParms(1).Value = Branchcode
        Try
            ds = SqlHelper.ExecuteDataset(connectionString, CommandType.StoredProcedure, "getBuyerdata", arParms)
        Catch ex As Exception
            MsgBox(ex.ToString)
        End Try
        If ds.Tables.Count = 0 Then
